...
<div class="sibot-fab">
  <div class="sibot-window" id="sibotWindow">
    <div class="sibot-header">
      <div class="sibot-avatar"><i class="fas fa-robot"></i></div>
      <div>
        <div class="sibot-name">SIBOT — Asisten SMKN 2</div>
        <div class="sibot-status">Online sekarang</div>
      </div>
      <i class="fas fa-times sibot-close" onclick="toggleSibot()"></i>
    </div>
    <div class="sibot-messages" id="sibotMessages">
      <div class="msg msg-bot">
        <div class="msg-avatar"><i class="fas fa-robot"></i></div>
        <div>
          <div class="msg-bubble">Halo! Saya SIBOT, asisten virtual SMK Negeri 2 Mojokerto. Ada yang bisa saya bantu?</div>
          <div class="msg-time">Sekarang</div>
        </div>
      </div>
    </div>
    <div class="sibot-quick">
      <button class="quick-btn" onclick="sendQuick('Jurusan apa saja?')"><i class="fas fa-graduation-cap"></i> Jurusan</button>
      <button class="quick-btn" onclick="sendQuick('Info PPDB 2025')"><i class="fas fa-file-signature"></i> PPDB</button>
      <button class="quick-btn" onclick="sendQuick('Ekskul apa saja?')"><i class="fas fa-people-group"></i> Ekskul</button>
      <button class="quick-btn" onclick="sendQuick('Jadwal sekolah')"><i class="fas fa-calendar-days"></i> Jadwal</button>
      <button class="quick-btn" onclick="sendQuick('Info PKL')"><i class="fas fa-building"></i> PKL</button>
      <button class="quick-btn" onclick="sendQuick('Kontak sekolah')"><i class="fas fa-phone"></i> Kontak</button>
    </div>
    <div class="sibot-input-row">
      <input type="text" class="sibot-input" id="sibotInput" placeholder="Ketik pertanyaan..." onkeydown="if(event.key==='Enter')sendMsg()" />
      <button class="sibot-send" onclick="sendMsg()"><i class="fas fa-paper-plane"></i></button>
    </div>
  </div>
  <button class="sibot-toggle" onclick="toggleSibot()">
    <i class="fas fa-robot" id="sibotIcon"></i>
    <div class="sibot-badge">1</div>
  </button>
</div>
...
<div class="a11y-widget">
  <div class="a11y-panel" id="a11yPanel">
    <div class="a11y-panel-title"><i class="fas fa-universal-access"></i> Pengaturan Aksesibilitas</div>

    <div class="a11y-group">
      <div class="a11y-group-label"><i class="fas fa-text-height"></i> Ukuran Teks</div>
      <div class="a11y-btn-row">
        <button class="a11y-btn" id="btnTextNormal" onclick="setTextSize('normal')">A</button>
        <button class="a11y-btn" id="btnTextLg" onclick="setTextSize('lg')">A+</button>
        <button class="a11y-btn" id="btnTextXl" onclick="setTextSize('xl')">A++</button>
      </div>
    </div>

    <div class="a11y-group">
      <div class="a11y-group-label"><i class="fas fa-circle-half-stroke"></i> Mode Kontras</div>
      <div class="a11y-btn-row">
        <button class="a11y-btn" id="btnContrastNormal" onclick="setContrast('normal')">Normal</button>
        <button class="a11y-btn" id="btnContrastDark" onclick="setContrast('dark')">Dark</button>
        <button class="a11y-btn" id="btnContrastHigh" onclick="setContrast('high')">Tinggi</button>
      </div>
    </div>

    <div class="a11y-group">
      <div class="a11y-group-label"><i class="fas fa-book-open"></i> Font Ramah Disleksia</div>
      <div class="a11y-toggle-row">
        <span class="a11y-toggle-label">Aktifkan font disleksia</span>
        <label class="a11y-switch">
          <input type="checkbox" id="dyslexicToggle" onchange="toggleDyslexic(this.checked)" />
    ...
    <div class="a11y-group">
      <div class="a11y-group-label"><i class="fas fa-arrows-up-down"></i> Jarak Antar Baris</div>
      <div class="a11y-btn-row">
        <button class="a11y-btn" id="btnLineNormal" onclick="setLineHeight('normal')">Normal</button>
        <button class="a11y-btn" id="btnLineWide" onclick="setLineHeight('wide')">Lebar</button>
        <button class="a11y-btn" id="btnLineXwide" onclick="setLineHeight('xwide')">Sangat Lebar</button>
      </div>
    </div>

    <button class="a11y-reset" onclick="resetA11y()"><i class="fas fa-undo"></i> Reset ke Default</button>
  </div>
  <button class="a11y-toggle" onclick="toggleA11y()" title="Pengaturan Aksesibilitas">
...
<script>
  ...
  /* ---- ACCESSIBILITY WIDGET ---- */
  function toggleA11y() { document.getElementById('a11yPanel').classList.toggle('open'); }

  function loadA11ySettings() {
    var s = JSON.parse(localStorage.getItem('a11y') || '{}');
    if(s.textSize) setTextSize(s.textSize, true);
    if(s.contrast) setContrast(s.contrast, true);
    if(s.dyslexic) { document.getElementById('dyslexicToggle').checked = true; toggleDyslexic(true, true); }
    if(s.lineHeight) setLineHeight(s.lineHeight, true);
  }

  function saveA11y(key, val) {
    var s = JSON.parse(localStorage.getItem('a11y') || '{}');
    s[key] = val;
    localStorage.setItem('a11y', JSON.stringify(s));
  }

  function setTextSize(size, silent) {
    document.body.classList.remove('a11y-text-lg','a11y-text-xl');
    document.querySelectorAll('#btnTextNormal,#btnTextLg,#btnTextXl').forEach(function(b){ b.classList.remove('active'); });
    if(size === 'lg') { document.body.classList.add('a11y-text-lg'); document.getElementById('btnTextLg').classList.add('active'); }
    else if(size === 'xl') { document.body.classList.add('a11y-text-xl'); document.getElementById('btnTextXl').classList.add('active'); }
    else { document.getElementById('btnTextNormal').classList.add('active'); }
    if(!silent) saveA11y('textSize', size);
  }

  function setContrast(mode, silent) {
    document.body.classList.remove('a11y-high-contrast','a11y-dark-mode');
    document.querySelectorAll('#btnContrastNormal,#btnContrastDark,#btnContrastHigh').forEach(function(b){ b.classList.remove('active'); });
    if(mode === 'high') { document.body.classList.add('a11y-high-contrast'); document.getElementById('btnContrastHigh').classList.add('active'); }
    else if(mode === 'dark') { document.body.classList.add('a11y-dark-mode'); document.getElementById('btnContrastDark').classList.add('active'); }
    else { document.getElementById('btnContrastNormal').classList.add('active'); }
    if(!silent) saveA11y('contrast', mode);
  }

  function toggleDyslexic(on, silent) {
    document.body.classList.toggle('a11y-dyslexic', on);
    if(!silent) saveA11y('dyslexic', on);
  }

  function setLineHeight(mode, silent) {
    document.body.classList.remove('a11y-line-wide','a11y-line-xwide');
    document.querySelectorAll('#btnLineNormal,#btnLineWide,#btnLineXwide').forEach(function(b){ b.classList.remove('active'); });
    if(mode === 'wide') { document.body.classList.add('a11y-line-wide'); document.getElementById('btnLineWide').classList.add('active'); }
    else if(mode === 'xwide') { document.body.classList.add('a11y-line-xwide'); document.getElementById('btnLineXwide').classList.add('active'); }
    else { document.getElementById('btnLineNormal').classList.add('active'); }
    if(!silent) saveA11y('lineHeight', mode);
  }

  function resetA11y() {
    localStorage.removeItem('a11y');
    document.body.classList.remove('a11y-text-lg','a11y-text-xl','a11y-high-contrast','a11y-dark-mode','a11y-dyslexic','a11y-line-wide','a11y-line-xwide');
    document.getElementById('dyslexicToggle').checked = false;
    document.querySelectorAll('.a11y-btn').forEach(function(b){ b.classList.remove('active'); });
    document.getElementById('btnTextNormal').classList.add('active');
    document.getElementById('btnContrastNormal').classList.add('active');
    document.getElementById('btnLineNormal').classList.add('active');
  }
  ...
  document.addEventListener('click', function(e) {
    if(!e.target.closest('.a11y-widget')) document.getElementById('a11yPanel').classList.remove('open');
  });
